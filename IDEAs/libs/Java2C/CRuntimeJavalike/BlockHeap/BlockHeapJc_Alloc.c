...
void activateGC_ObjectJc(void* ythis, void* retObject, ThCxt* _thCxt)
{
  activateGarbageCollectorAccess_BlockHeapJc(ythis, retObject);
}
...
{ BlockHeapBlockJc* retBlock;
  ...
  do {
    ...
    { memset(retBlock, 0, sizeBlock);
      retBlock->typeOrMaxRef = maxRef;
      #ifdef __RUNTIMEHEAP_TEST__
        retBlock->references__debug = null;
      #endif
      retBlock->heap = ythis;
      retBlock->heapidxAndMaxref = maxRef + (ythis->idxHeapCtrl << kBitHeapIdx_ObjectJc_BlockHeap);
...
{ MemC retMem;
  ...
  { BlockHeapJc* retHeap;
    ...
    { int nrofBytes;
      ...
      block->typeOrMaxRef = (block->typeOrMaxRef & ~ mMaxRef_BlockHeapJc) | nrofBackRefs;
