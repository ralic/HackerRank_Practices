...
{ //NOTE: it may be a 16-bit-integer, therefore shift and mask first.
  int idxBackref = (int)( (*refbase) >>kBitBackRef_ObjectJc) & (uint)(mBackRef_ObjectJc>>kBitBackRef_ObjectJc);
 ...
 */
static int getIdxMtbl(ClassJc const* reflectionObj, ClassJc const* reflectionRef)
{ int idxMtbl = -1;
    ...
    { int idxSuper = 0;
      ...
      { ClassOffset_idxMtblJc const* reflectionChild;
        ...
        { //Recursive call because deeper inheritance:
          idxMtbl = getIdxMtbl(reflectionChild->clazz, reflectionRef);
        }
      }
    }
...
*/
bool setBackRefIdxMtblJc(ObjectRefValuesJc* refbase, void const* src, ClassJc const* reflectionRef) //uint16 idxMtbl)
{ clearBackRefJc(refbase);

  if(src != null)
  { /*
     * NOTE: unused since 2009.10 because the idxMtbl can be disturbed in the data, it is unsafe.
    reflectionObj = src->reflectionClass;
    idxMtbl = getIdxMtbl(reflectionObj, reflectionRef);
    if(idxMtbl < 0)
    { STACKTRC_ENTRY("setBackrefIdxvtblJc");
      THROW_s0(ClassCastException, "classCastException", idxMtbl);
      STACKTRC_LEAVE;
    }
    */
    setBackRefJc(refbase, src);
    //*refbase |= idxMtbl;
  }
  //objRef->ref = src;
  return true;
  ...
  { int idx = 0;
    ...
    { if(*backRef == null)
      { /**NOTE: ObjectRefValuesJc is the type of the reference-value.
        ...
        ObjectRefValuesJc refSave = *refbase & ~mBackRef_ObjectJc; 
