...
static void Win_2_OS(SOCKADDR_IN* sockaddr, OS_SOCKADDR* name)
{
...
static void OS_2_Win(OS_SOCKADDR* name, SOCKADDR_IN* sockaddr)
{
...
int os_setsockopt(OS_SOCKET so, int level, int optname, int* optval, int optlen)
{
	...
	if (optname == SO_LINGER){
		LINGER optvalWin;
		optvalWin.l_onoff = ((OS_LINGEROPT*)optval)->linger_on;
		optvalWin.l_linger = ((OS_LINGEROPT*)optval)->linger_time;  
