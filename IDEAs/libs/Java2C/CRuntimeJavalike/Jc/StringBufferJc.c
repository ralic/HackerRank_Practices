...
{ StringBuilderJc* ythis = PTR_MemC(rawMem, StringBuilderJc);
  ...
  ythis->size = size; //positive value because immediately String  
...
{ StringBuilderJc* ythis = (StringBuilderJc*)othis;
  ...
  { //enaugh size for StringBuilderJc-control structure and the buffer itself:
    ythis->size = size;  //positiv: use value as buffer direct
...
{ int sizeObj = getSizeInfo_ObjectJc(othis);
  ...
  { /**The StringBuffer has not a direct Buffer: */
    ...
    { ythis->value.buffer = PTR_MemC(mem, char);
      ythis->size = -sizeBuffer; //negative value because immediately String  
    }
    else
  ...
  else
  {
    ...
    ythis->size = sizeBuffer; //positive value because immediately String  
  }
  STACKTRC_LEAVE; return ythis;
}
...
StringBuilderJc* ctorO_s_StringBuilderJc(ObjectJc* othis, StringJc src, ThCxt* _thCxt)
{ StringBuilderJc* ythis;
  int zSrc = VAL_StringJc(src) & mLength__StringJc;
  if(zSrc == mLength__StringJc){ zSrc = strlen_Fwc(PTR_StringJc(src), mLength__StringJc); }
  ythis = ctorO_I_StringBuilderJc(othis, zSrc, _thCxt);
  return append_s_StringBuilderJc(ythis, src, _thCxt);
}
...
int copyToBuffer_StringBuilderJc(StringBuilderJc* ythis, char* buffer, int zBuffer)
{ int count;
  char const* src;
...
{ ythis->_count = (ythis->_count & ~mLength__StringJc) | count;
}
...
{ int valueSrc = value_OS_PtrValue(src);
  ...
  if(valueSrc & mNonPersists__StringJc){  
    ...
    set_OS_PtrValue(*ythis, sSrc, zSrc);
  } else {
    //without copy to a new buffer
    set_OS_PtrValue(*ythis, sSrc, zSrc);
  }
  setBackRefJc(&ythis->value__, PTR_OS_PtrValue(*ythis, char const));
  STACKTRC_LEAVE;
}
...
void old_setRef_StringJc(StringJc* ythis, StringJc src)
{ int srcLength;
  char const* srcString;
  STACKTRC_ENTRY("setRef_StringJc");
  srcString = getCharsAndLength_StringJc(&src, &srcLength);
  if(src.value__ & mNonPersists__StringJc){
    /**if the String is not persistent, it should be copied! */
    StringBuilderJc* buffer = new_StringBuilderJc(-1, _thCxt); //allocate in heap
    replace_zI_StringBuilderJc(buffer, 0,0, srcString, srcLength, _thCxt); //copy
    srcString = getCharsAndCount_StringBuilderJc(buffer, &srcLength); 
  }
  if(srcString == null)
  { /**handle null-pointer. */
    clearBackRefJc(&ythis->value__);  
    clear_StringJc(ythis);  
  }
  else 
  {
    set_OS_PtrValue(*ythis, srcString, srcLength);  //yet without backref
    #ifdef mBackRef_ObjectJc
      setBackRefJc(&ythis->value__, srcString);  //clears also an existing backref.
    #endif
  }
  STACKTRC_LEAVE;
}
...
{
  StringJc ret;
  int value = value_OS_PtrValue(src);
...
{
  int value = value_OS_PtrValue(ythis);
  value &= ~mNonPersists__StringJc;
  setValue_OS_PtrValue(ythis, value);
  return ythis;
}
