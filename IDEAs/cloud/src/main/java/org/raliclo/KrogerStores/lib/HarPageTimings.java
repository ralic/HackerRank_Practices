...
public class HarPageTimings {
     ...
     * @throws IOException
     */
    public HarPageTimings(JsonParser jp, List<HarWarning> warnings) throws IOException {
    ...
    public HarPageTimings(HarDatabaseConfig config, long pageId) throws SQLException {
        ...
        try {
            ps = c.prepareStatement(
                    "SELECT page_timings_id,on_content_load,on_load,comment FROM " + tableName + " WHERE page_id=?");
            ps.setLong(1, pageId);
            rs = ps.executeQuery();
            if (!rs.next())
    ...
    public String toString() {
        return "\"pageTimings\": { \"onContentLoad\": " + onContentLoad + ",\"onLoad\":" + onLoad + ", \"comment\": \""
