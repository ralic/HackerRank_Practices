...
object SpectralNorm_scala2 {
  ...
  private def spectralnormGame(n: Int): Double = {
    ...
    val ap: Array[SpectralNorm_scala2.Approximate] = new Array[SpectralNorm_scala2.Approximate](nthread)
    i = 0
    while (i < nthread) {
      {
        val r1: Int = i * chunk
        ...
        ap(i) = new SpectralNorm_scala2.Approximate(u, v, tmp, r1, r2)
      }
      {
        i += 1;
        i - 1
  ...
  private object Approximate {
    ...
    private def eval_A(i: Int, j: Int): Double = {
  ...
  private class Approximate(var _u: Array[Double], var _v: Array[Double], var _tmp: Array[Double], var range_begin: Int, var range_end: Int) extends Thread {
    ...
    final private def MultiplyAv(v: Array[Double], Av: Array[Double]) {
      var i: Int = range_begin
      while (i < range_end) {
        {
          var sum: Double = 0
    ...
    final private def MultiplyAtAv(v: Array[Double], tmp: Array[Double], AtAv: Array[Double]) {
      try {
        MultiplyAv(v, tmp)
